...
from skill_risk_manager.backend.cache import file_hash, stat_matches_cache
from skill_risk_manager.backend.classifier import Classification, classify_path
from skill_risk_manager.backend.fast_exit import FastExitTracker, source_group_for_target
from skill_risk_manager.backend.models import (
    CancelToken,
...
from skill_risk_manager.backend.parser import parse_markdown_header
from skill_risk_manager.backend.priority_queue import ScanPriorityQueue
from skill_risk_manager.backend.scanner_utils import (
    build_cache_record,
    cached_classification,
    cached_record_with_risk,
    deduplicate_records,
    ...
    def _record_from_cache_or_parse(
        ...
        if stat_matches_cache(path, cache_record):
            ...
            if raw_cached_classification:
                return cached_record_with_risk(raw_cached_classification, self.risk_policy)
        ...
        if computed_hash and cache_record and computed_hash == cache_record.get("hash"):
            raw_cached_classification = cached_classification(cache_record)
            if raw_cached_classification:
                cached_record = cached_record_with_risk(raw_cached_classification, self.risk_policy)
        ...
        if classification.status != "ignored" and classification.confidence > 0:
            ...
            record = record_with_risk(record, self.risk_policy)
